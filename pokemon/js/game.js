// ===== Canvas =====
const canvas = document.getElementById("gameCanvas"); //canvasを使うために、htmlの要素を取得
const ctx = canvas.getContext("2d"); //canvasに描画するためのオブジェクトを取得
...
maps.map1.src = "image/home_2F.png";
maps.map2.src = "image/home_1F.png";   
maps.map3.src = "image/masara_town.png";
maps.map4.src = "image/ookido.png";

let currentMap = "map1";

const battleBg = new Image();
battleBg.src = "image/battle.png";

// ===== BGM =====
//マサラタウン
const bgm = new Audio("sound/masara_town.mp3");
bgm.loop = true;   // ループ再生
bgm.volume = 0.3; // 音量（0〜1）
//オーキド研究所
const bgmMap4 = new Audio("sound/ookido.mp3"); // map4用
bgmMap4.loop = true;
bgmMap4.volume = 0.3;
//効果音
const seWarp = new Audio("sound/warp.mp3"); // ←ファイル名
seWarp.volume = 0.3;
...
playerImages.down[0].src = "image/player_down_1.png";
playerImages.down[1].src = "image/player_down_2.png";

playerImages.up[0].src = "image/player_up_1.png";
playerImages.up[1].src = "image/player_up_2.png";

playerImages.left[0].src = "image/player_left_1.png";
playerImages.left[1].src = "image/player_left_2.png";

playerImages.right[0].src = "image/player_right_1.png";
playerImages.right[1].src = "image/player_right_2.png";

// 戦闘中ポケモン画像_味方側
const battlePlayerImg = new Image();
battlePlayerImg.src = "image/フシギダネ.png";
...
function updatePlayer(){
  ...
  if(Math.abs(player.x - player.targetX) < speed &&
    ...
    warps.forEach(w => {
        ...
        seWarp.currentTime = 0;
        seWarp.play();
        currentMap = w.toMap;
        if(currentMap === "map4"){
          if(bgmMap4.paused){
            bgm.pause();
            bgmMap4.currentTime = 0;
            bgmMap4.play();
          }
        } else {
          if(bgm.paused){
...
function drawBattle(){
  ...
  if(battleBg.complete){
    ctx.drawImage(battleBg, 0, 0, 400, 400);
  }
  ...
  if(battlePlayerImg.complete){
    ctx.drawImage(battlePlayerImg, 80, 160, 80, 80);
  }
...
function loop(){
  ...
  requestAnimationFrame(loop);//ブラウザの描画サイクルに合わせて、loopを呼ぶ。
}
